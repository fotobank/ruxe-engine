     	...
     	switch ($action)
     	{
        	...
        	case "index":
  			...
  			if ($creator)
  			{
       				$addinfo="";
       				if ($count_new>0)
            				$addinfo.="<br>У Вас <a style=\"color:black; font-weight:bold;\" href=\"?action=newmessages\">".$Filtr->truecount($count_new,"непрочитанное сообщение","непрочитанных сообщения","непрочитанных сообщений")."</a>. Думаю, с этого стоит и начать.";
       				if ((date("d.m")=="11.02") && ($Filtr->tolower($login)=="includen"))
            					$addinfo.="<br>Поздравляю с днём рождения, Создатель. Желаю счастья, здоровья, успехов и много много денег.";
       				if ((date("d.m")=="14.10") && ($Filtr->tolower($login)=="dr1d"))
            					$addinfo.="<br>Поздравляю с днём рождения, Создатель. Желаю счастья, здоровья, успехов и много много денег.";
       				if (date("d.m")=="21.04")
            					$addinfo.='<br>Завтра у меня день рождения. Надо сообщить пользователям об этом, Создатель.';
       				if (date("d.m")=="22.04")
            					$addinfo.="<br>Сегодня у меня день рождения =)";
       				$addinfo.='<script type="text/javascript">
            					today = new Date(); 
            					if (today.getHours() >= 22)
            					{ 
                    					document.write("<br>Создатель, Вам пора готовиться ко сну. Завтра рано вставать.");
            					}
            					if (today.getHours() <= 5)
            					{ 
                    					document.write("<br>Почему Вы всё ещё не спите? Это меня беспокоит.");
            					}
            					</script>';
       				$t = ($today_hosts-$last_hosts[0]);
       				if ($t>2)
            					$addinfo.="<br>Сегодня сайт посетило на ".$Filtr->truecount($t,"человека","человека","людей")." больше, чем вчера. Это успех.";

       				include($cms_root."/conf/backup.php");
       				$views    = file($cms_root."/conf/views.dat");
       				$bviews   = explode("|",$backup_views);
       				$i=0;
       				foreach ($bviews as $b)
       				{
                 				$a = explode("=",$b);
                 				...
                 				if ($a[0]!=$c[0])
                 				{
                   					$addinfo.="<br>Похоже нарушена структура счётчиков просмотров. Не хватает ".$a[0].".";
                 				}
                 				else
                 				{
                  					if ((int)str_replace("\r\n","",$c[1])<(int)str_replace("\r\n","",$a[1]))
                  					{
                   						$addinfo.='<br>Счётчик просмотров '.$a[0].' обнулён.';
                  					};
                 				};
                 				$i++;
       				};
       				...
       				$bdownloads   = explode("|",$backup_downloads);
       				$i=0;
       				foreach ($bdownloads as $b)
       				{
                 				$a = explode("=",$b);
                 				$c = explode("=",$downloads[$i]);
                 				if ($a[0]!=$c[0])
                 				{
                 					$addinfo.="<br>Похоже нарушена структура счётчиков скачиваний. Не хватает ".$a[0].".";
                 				}
                 				else 
                 				{
                 					if ((int)str_replace("\r\n","",$c[1])<(int)str_replace("\r\n","",$a[1]))
                  					{
                   						$addinfo.='<br>Счётчик скачиваний '.$a[0].' обнулён.';
                 					};
                 				};
                 				$i++;
       				};
       				$install .= '
       						<tr class="titleblue"><td colspan=2>СИСТЕМА РАДА ВИДЕТЬ ВАС, СОЗДАТЕЛЬ</td></tr>
       						<tr class="bluetd"><td colspan=2>Жду Ваших указаний, Создатель.'.$addinfo.'</td></tr>
       					';
  			};
			/*
