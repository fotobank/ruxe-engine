...
<table class="optionstable" border=0 cellpadding=1 cellspacing=0>
	...
	<tr class="titletable"><td colspan=2>СИСТЕМНОЕ
		...
		<tr><td>Адрес сайта:<br><font class="desc">Пример: <b>http://site.com</b></font></td><td><input id="jsite" onKeyUp="generalwwwredirect();" type="text" name="site" value="{SITE}"></td></tr>
		...
		<tr><td>Полный путь до скриптов:<br><font class="desc">Пример: <b>/home/mysite/public_html</b></font></td><td><input type="text" name="root" value="{ROOT}"></td></tr>
		...
		<tr><td>Сообщение, выводимое при закрытом сайте:<br><font class="desc">HTML-теги разрешены</font></td><td><textarea name="closed_text">{CLOSED_TEXT}</textarea></td></tr>
	
	<tr class="titletable"><td colspan=2>НОВОСТНЫЕ РАЗДЕЛЫ</td></tr>
		<tr><td>Расширение для полных записей новостных разделов:<br><font class="desc">Пример: <b>.html</b>. Актуально только при активном ЧПУ.</font></td><td><input type="text" name="rewrite_ext" value="{REWRITE_EXT}"></td></tr>
	...
	<tr class="titletable"><td colspan=2>КОММЕНТАРИИ</td></tr>
		...
		<tr><td>Количество одобренных сообщений, после которых перестаёт действовать премодерация на пользователя:<font class="desc"><br>Актуально только при включённой премодерации, на администраторов не действует</font></td><td><input type="text" name="premod_mess" value="{premod_mess}"></td></tr>
		...
		<tr><td>Возможность использовать тег [img] (не рекомендуется):</td><td><select name="img_comments"><option value="1" {IMG_COMMENTS1}>Да<option value="0" {IMG_COMMENTS2}>Нет</select></td></tr>
                <tr><td>Темы сообщений для модуля Обратной связи:<br><font class="desc">Разделяйте запятой, без пробела после неё</font><td><input type="text" name="mail_select" value="{MAIL_SELECT}"></td></tr>  
		...
		<tr><td>Длина сообщений в модуле <font class="desc">&lt;?php here_last_posts(); ?&gt;</font> (в символах):{SUBSTRERROR}</td><td><input type="text" name="lastposts_len" value="{LASTPOSTS_LEN}"></td></tr>
		...
		<tr><td>Цензура: часть слов, заменяющиеся на <font class="desc">***</font>, <b>через запятую БЕЗ пробела и маленькими буквами</b>:<br><font class="desc">Не действует на администраторов</font></td><td><input type="text" name="cenzura_words" value="{CENZURA_WORDS}"></td></tr> 
	...
	<tr class="titletable"><td colspan=2>АВАТАРЫ</td></tr>
		...
		<tr><td>Максимальный вес аватары (килобайты):<font class="desc"><br>При включённой опции автоматического изменения размера изображения <b>не учитывается</b></font></td><td><input type="text" name="upload_maxsite" value="{UPLOAD_MAXSITE}"></td></tr>
		<tr><td>Максимально допустимая ширина аватары (пикселы): </td><td><input type="text" name="upload_width" value="{UPLOAD_WIDTH}"></td></tr>
		<tr><td>Максимально допустимая высота аватары (пикселы): </td><td><input type="text" name="upload_height" value="{UPLOAD_HEIGHT}"></td></tr>
		...
		<tr><td>Максимальный вес оригинального изображения (килобайты):<font class="desc"><br>Параметр <b>используется только</b> при включённой опции автоматического изменения размера изображения</font></td><td><input type="text" name="maxresize" value="{MAXRESIZE}"></td></tr>
		...
		<tr><td>Выводимый аватар, если не найден на сервисе <a href="http://gravatar.com" target="_blank">gravatar.com</a>:<br><font class="desc">Активен только в том случае, если используется сервис <a href="http://gravatar.com" target="_blank">gravatar.com</a>. Возможные варианты: <b>mm</b>, <b>identicon</b>, <b>monsterid</b>, <b>wavatar</b>, <b>retro</b></font></td><td><input type="text" name="gravatars_im" value="{GRAVATARS_IM}"></td></tr>
	...
	<tr class="titletable"><td colspan=2>ПОДПИСИ ПОЛЬЗОВАТЕЛЕЙ</td></tr>
		...
		<tr><td>Количество символов, разрешённых в подписи:</td><td><input type="text" name="ps_max" value="{PS_MAX}"></td></tr> 
  
	<tr class="titletable"><td colspan=2>EMAIL</td></tr>
		<tr><td>Ваш email адрес:</td><td><input type="text" name="mail" value="{MAIL}"></td></tr>
		<tr><td>Email адрес, который будет считаться отправителем уведомлений о новых комментариях (в т.ч. обратной связи):</td><td><input type="text" name="send_mail" value="{SEND_MAIL}"></td></tr>
		...
		<tr><td>Email адрес, который будет считаться отправителем писем с кодом активации и восстановлением пароля:</td><td><input type="text" name="active_mail" value="{ACTIVE_MAIL}"></td></tr>
		<tr><td>Имя отправителя писем с кодом активации и восстановлением пароля:</td><td><input type="text" name="active_name" value="{ACTIVE_NAME}"></td></tr>
	...
	<tr class="titletable"><td colspan=2><a name="editor"></a>РЕДАКТОР В АДМИН-ЦЕНТРЕ</td></tr>
		...
		<tr><td>Шрифт, используемый в редакторе EditArea:<br><font class="desc">Пример: <b>Courier New</b></font></td><td><input type="text" name="fontfamily" value="{FONTFAMILY}"></td></tr>
	...
	<tr class="titletable"><td colspan=2>БЕЗОПАСНОСТЬ</td></tr>
		...
		<tr><td>Время жизни сессии пользователей и администраторов (в секундах):</td><td><input type="text" name="time_cookie" value="{TIME_COOKIE}"></td></tr>
		...
		<tr><td>Защита от флуда (секунды):<font class="desc"><br>Не действует на администраторов</font></td><td><input type="text" name="noflood" value="{NOFLOOD}"></td></tr>
		...
		<tr><td>Максимальное количество символов в комментариях:{SUBSTRERROR3}</td><td><input type="text" name="max_message" value="{MAX_MESSAGE}"></td></tr>
  	...
  	<tr class="titletable"><td colspan=2>RSS</td></tr>
		<tr><td>Заголовок RSS ленты:</td><td><input type="text" name="rsstitle" value="{RSSTITLE}"></td></tr>
		<tr><td>Часовой пояс RSS каналов:</td><td><input type="text" name="rss" value="{RSS}"></td></tr>
	...
	<tr class="titletable"><td colspan=2>СТАТИСТИКА</td></tr>
		...
		<tr><td>Максимальное количество записей в статистике посещений:</td><td><input type="text" name="log_max" value="{LOG_MAX}"></td></tr>
	...
	<tr class="titletable"><td colspan=2>ДРУГОЕ</td></tr>
		<tr><td><input type="hidden" name="nav_downloads">Записей F.A.Q. на одной странице:</td><td><select name="nav_faq">{NAV_FAQ}</select></td></tr>
		...
		<tr><td>Whois сервис, используемый в админ-центре:<font class="desc"><br>В конец адреса подставляется IP, поэтому следует учесть параметры типа ?q=, ?ip= и др.</font></td><td><input type="text" name="whois" value="{WHOIS}"></td></tr>
